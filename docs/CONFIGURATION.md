...
| `INFLUXDB_URL` | InfluxDB v2 base URL (optional) | `http://localhost:8086` |
| `INFLUXDB_TOKEN` | InfluxDB v2 API token (optional) | `my-token` |
| `INFLUXDB_ORG` | InfluxDB organisation name (optional, often empty) | `` |
| `INFLUXDB_BUCKET` | InfluxDB bucket name (optional) | `brewery` |
...
`LOGO_PNG` / `LOGO_SVG` are copied from `input/logo/` to `web/logo/` on each export run.  
InfluxDB variables are only required when running `fetch_influxdb.py` — all other scripts ignore them.
